 ...
 * 
 * @see{WeakHashMap} from JDK 1.5. This modification uses
 *                   System.identityHashCode() rather than the object's hash
 *                   code. All equality checks are identity checks (==) rather
 *                   than objet equality (.equals);
 * @see{IdentityHashMap} for more information on the changes made in an identity
 *                       hash map.
 * 
 * A hashtable-based <tt>Map</tt> implementation with <em>weak
 * keys</em>.
 * An entry in a <tt>WeakIdentityHashMap</tt> will automatically be removed
 * when its key is no longer in ordinary use. More precisely, the presence of a
 ...
 * <tt>WeakIdentityHashMap</tt> may be constructed using the
 * <tt>Collections.synchronizedMap</tt> method.
 * 
 * <p>
 * The behavior of the <tt>WeakIdentityHashMap</tt> class depends in part upon
 * the actions of the garbage collector, so several familiar (though not
 * required) <tt>Map</tt> invariants do not hold for this class. Because the
 * garbage collector may discard keys at any time, a
 * <tt>WeakIdentityHashMap</tt> may behave as though an unknown thread is
 * silently removing entries. In particular, even if you synchronize on a
 * <tt>WeakIdentityHashMap</tt> instance and invoke none of its mutator
 * methods, it is possible for the <tt>size</tt> method to return smaller
 * values over time, for the <tt>isEmpty</tt> method to return <tt>false</tt>
 * and then <tt>true</tt>, for the <tt>containsKey</tt> method to return
 * <tt>true</tt> and later <tt>false</tt> for a given key, for the
 * <tt>get</tt> method to return a value for a given key but later return
 * <tt>null</tt>, for the <tt>put</tt> method to return <tt>null</tt> and
 * the <tt>remove</tt> method to return <tt>false</tt> for a key that
 ...
 * 
 * <p>
 * Each key object in a <tt>WeakIdentityHashMap</tt> is stored indirectly as
 ...
 * <tt>WeakIdentityHashMap</tt> are held by ordinary strong references. Thus
 ...
 * keys from being discarded. Note that a value object may refer indirectly to
 * its key via the <tt>WeakIdentityHashMap</tt> itself; that is, a value
 * object may strongly refer to some other key object whose associated value
 * object, in turn, strongly refers to the key of the first value object. One
 * way to deal with this is to wrap values themselves within
 * <tt>WeakReferences</tt> before inserting, as in:
 * <tt>m.put(key, new WeakReference(value))</tt>, and then unwrapping upon
 * each <tt>get</tt>.
 * 
 * <p>
 ...
 */
public class WeakIdentityHashMap<K, V> extends AbstractMap<K, V> implements
		Map<K, V> {
	 ...
	 */
	// not: "private static <K> K maskNull(K key)" because NULL_KEY isn't of
	// type K.
