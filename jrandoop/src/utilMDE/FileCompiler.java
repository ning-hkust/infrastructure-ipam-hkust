 ...
 * source. It invokes a user-specified external command, such as <tt>javac</tt>
 * or <tt>jikes</tt>.
 */
public final class FileCompiler {
	...
	static {
		try {
			java_filename_pattern = Pattern.compile("([^"
	 ...
	 * 
	 * @param compiler
	 *            A command that runs a Java compiler; for instance, it could be
	 *            the full path name or whatever is used on the commandline.
	 * @param timeLimit
	 *            The maximum permitted compilation time, in msec.
	 */
	 ...
	 * 
	 * @param fileNames
	 *            pathes to the files to be compiled as Strings.
	 */
	public void compileFiles(List<String> fileNames) {
		...
		try {
			...
			if (p.timed_out())
				System.out.println("Compile timed out after "
						+ p.timeout_msecs() + " msecs");
	...
	/**
	 * @param filename
	 *            the path of the java source to be compiled
	...
	/**
	 * @param filenames
	 *            the paths of the java source to be compiled as Strings.
	...
	/**
	 * Examine the errorString to identify the files that cannot compile, then
	 * recompile all the other files. This function is necessary when compiling
	 * with javac because javac does not compile all the files supplied to it if
	 * some of them contain errors. So some "good" files end up not being
	 * compiled.
	 */
	private void recompile_without_errors(List<String> fileNames,
			String errorString) {
		...
		if (errorString != null) {
			...
			if (retry.size() > 0) {
				...
				try {
					...
					System.out.println("Compile of " + filenames
							+ " interrupted: " + e);
				}
			}
		}
	...
	/**
	 * Return the file path to where a class file for a source file at
	 * sourceFilePath would be generated.
	 */
	private static String getClassFilePath(String sourceFilePath) {
		...
		if (index == -1) {
			throw new IllegalArgumentException("sourceFilePath: "
					+ sourceFilePath + " must end with an extention.");
		}
	...
	/**
	 * Returns the name of the class that sourceFilePath is a path for.
	 */
