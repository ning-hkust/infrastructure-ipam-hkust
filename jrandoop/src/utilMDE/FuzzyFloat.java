...
/**
 * Routines for doing 'fuzzy' floating point comparisons. Those are comparisons
 * that only require the floating point numbers to be relatively close to one
 * another to be equal, rather than exactly equal.
 * <p>
 * 
 * Floating point numbers are compared for equality by dividing them by one
 * another and comparing the ratio. By default they must be within 0.0001
 * (0.01%) to be considered equal; set this value with set_rel_diff. Note that
 * zero is never equal to a non-zero number using this method.
 * <p>
 * 
 * Additionally two NaN values are always considered equal.
 * <p>
 */
...
public class FuzzyFloat {
	...
	/**
	 * Specify the specific relative difference allowed between two floats in
	 * order for them to be equal. The default is 0.0001 a relative diff of
	 * zero, disables it (i.e., only exact matches work).
	 */
	...
	/**
	 * Test d1 and d2 for equality using the current ratio. Two NaN floats are
	 * not considered equal (consistent with the == operator).
	 * <p>
	 * 
	...
	/**
	 * Searches for the first occurrence of elt in a. elt is considered equal to
	 * a[i] if it passes the {@link #eq} test.
	 * 
	 ...
	 * Elements of sub are considered to match elements of a if they pass the
	 * {@link #eq} test.
	 * 
	 * @return the first index whose subarray is equal to the specified array or
	 *         -1 if no such subarray is found in the array.
	 * @see java.util.Vector#indexOf(java.lang.Object)
	 ...
	 * same elements). Element comparison uses {@link #eq}.
	 * <p>
	 * 
	 * Note that this implementation is optimized for cases where the elements
	 * are actually the same, since it does a sort of both arrays before
	...
	/* @ pure */public class DoubleArrayComparatorLexical implements
			Comparator<double[]> {
	...
	/**
	 * Determines whether smaller is a subset of bigger. Element comparison uses
	 * {@link #eq}.
	 * <p>
	 * 
	 * Note that this implementation is optimized for cases where the elements
	 ...
	 * starting the comparisons.
	 * 
	 * @return true if smaller is a subset (each element of smaller is also a
	 *         element of bigger) of bigger, false otherwise
	 */
