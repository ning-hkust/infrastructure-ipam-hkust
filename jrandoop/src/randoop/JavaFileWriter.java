...
public class JavaFileWriter {
	...
	public static void createJavaFiles(List<Sequence> sequences,
			String junitClassName, int testsPerFile) {
		if (sequences.size() == 0) {
			System.out
					.println("No sequences given to createJavaFiles. No files created.");
			return;
		}
	...
	private static void writeSubSuite(List<Sequence> sequencesForOneFile,
			int i, String junitClassName) {
	...
	private static void writeDriverFile(int numSubSuites, String junitClassName) {
		...
		try {
			...
			out
					.println("    System.out.println(\"Executed \" + numTests + \" tests.\");");
			out
					.println("    System.out.println(\"Time spent executing tests: \" + (stopTime-startTime) + \"ms.\");");
			out.println("  }");
			out.println("");
			out.println("}");
		} finally {
