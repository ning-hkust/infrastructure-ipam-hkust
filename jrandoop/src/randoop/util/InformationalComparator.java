...
public class InformationalComparator<T> {
	...
	public ComparisonResult compare(String l1Name, SimpleList<T> l1,
			String l2Name, SimpleList<T> l2) {
		...
		if (equalSize(l1, l2)) {
			...
			b.append("Lists differ in size. List " + l1Name + " has size "
					+ l1.size() + " and list " + l2Name + " has size "
					+ l2.size() + Globals.lineSep);
		}
		...
		for (int i = 0; i < lastIndex; i++) {
			...
			if (i < l1.size() && i >= l2.size()) {
				...
				b
						.append("Smaller suite is equal to larger suite up to its last index.");
				break;
			}
