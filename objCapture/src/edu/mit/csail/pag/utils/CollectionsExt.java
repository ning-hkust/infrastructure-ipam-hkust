...
public final class CollectionsExt {
    ...
    /**
     * Make a copy of the list with duplicates removed. The first elements of each equivalence class will be retained
    ...
    public static <T, V> Map<T, V> diff(Map<? extends T, V> m1, Map<? extends T, V> m2) {
        Map<T, V> result = new LinkedHashMap<T, V>();
        for (Map.Entry<? extends T, V> pair : m1.entrySet()) {
            T key = pair.getKey();
    ...
    /**
     * Returns an ArrayList that is a concatenation of the arguments. Elements of arguments are copied, the argument
    ...
    /**
     * Returns an ArrayList in which the element is appended to the argument. Lists are copied.
     */
    @SuppressWarnings("unchecked")
    public static <T> List<T> append(List<? extends T> list, T element) {
        return concat(list, Collections.<T> singletonList(element));
    }
    ...
    public static String toStringInLines(Map<?, ?> m) {
        StringBuilder b = new StringBuilder();
        b.append("{").append(Util.newLine);
        for (Entry<?, ?> e : m.entrySet()) {
            b.append(e).append(Util.newLine);
        }
        b.append("}").append(Util.newLine);
        return b.toString();
    }
    ...
    /**
     * Prints out the String.valueOf() of all elements of the collection, inserting a new line after each element. The
     * order is specified by the collection's iterator.
     */
    public static String toStringInLines(Collection<?> c) {
        if (c.isEmpty())
            return "";
        return join(Util.newLine, toStringLines(c)) + Util.newLine;
    }

    /**
     * Prints out the elements of the collection in lines, in lexicographic order of String.valueOf called on each
     * element.
     */
    public static String toStringInSortedLines(Collection<?> c) {
        if (c.isEmpty())
            return "";
        return join(Util.newLine, sort(toStringLines(c))) + Util.newLine;
    }

    /**
     * List of String.valueOf() of all elements of the collection. The order is specified by the collection's iterator.
     */
    ...
    /**
     * Divides the argument into chunks of at most the given length. All chunks except at most one will have length
     * exactly maxLength. No chunks are empty.
     * 
     * The result list is unmodifiable. It does <em>not</em> copy the list and simply shares it.
     */
    ...
    public static <T> Set<T> getAll(Enumeration<? extends T> e) {
        Set<T> result = new LinkedHashSet<T>();
    ...
    public static <T> Set<T> getAll(Iterator<? extends T> iter) {
        Set<T> result = new LinkedHashSet<T>();
        while (iter.hasNext()) {
    ...
    public static <T> List<T> getAllAsList(Iterator<? extends T> x) {
    ...
    /**
     * Removed from the collection all strings that match the given pattern. Returns the modified collection, for easier
     * chaining.
     */
    ...
    /**
     * Creates a set of all sequences of length max of objects. The order matters and elements may be repeated. The set
     * is grouped by the number of objects in the array. The result is a map from arities to object sequences.
     * 
     * NOTE: This is done with arrays rather than lists because in JOE we don't want to execute any code of the objects.
     */
    ...
    /**
     * Creates and returns an array that contains all elements of the array parameter and has the el parameter appened.
     * 
    ...
    /**
     * For a given list of list of objects, create and return the (exponential in size) list of list of objects that
     * comes from combining the every element from the first list with every element from the second list etc. Eg for
     * input [[1,2],[a,b]] this returns [[1,a][1,b],[2,a],[2,b]]
     * 
     * For empty input, returns a list with one element: empty list. Requires: all lists non-empty
     */
    ...
    /**
     * Returns true iff the collection contains the element. Objects are compared by identity (==).
     */
    ...
    /**
     * Inverts a map. Only works for infective maps.
     */
    public static <K, V> Map<V, K> invert(Map<K, V> map) {
        ...
        for (Entry<K, V> e : map.entrySet()) {
            ...
            if (result.containsKey(v))
                throw new IllegalArgumentException("Multiple keys for value:" + v);
    ...
    /**
     * Returns a copy of the relation, with only key and values that satisfy the predicate.
     */
    public static <K> Relation<K, K> view(Relation<K, K> rel, Predicate<K> pred) {
        Relation<K, K> clone = rel.clone();
        Predicate<K> not = Predicate.NOT(pred);
        clone.removeKeys(not);
        clone.removeValues(not);
        return clone;
    }
