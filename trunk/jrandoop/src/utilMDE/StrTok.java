 ...
 * <p>
 * 
 * The major difference from StreamTokenizer is that all tokens are returned as
 * strings. EOF returns a null, EOL returns an empty string. Delimiters are
 * returned as one character strings. Words and numbers are returned as strings.
 * Quoted strings are also returned as strings including their quote characters
 * (so they can easily be differentiated from words and numbers).
 * 
 ...
 * converted to RuntimeExceptions so that every call doesn't have to be included
 * in a try block
...
public class StrTok {
	...
	/**
	 * Returns the next token as a string. EOF returns a null, EOL returns an
	 * empty string. Delimiters are returned as one character strings. Quoted
	 * strings and words are returned as strings.
	 */
	...
	/**
	 * Sets the error handler. The default error handler will throw a runtime
	 * exception on errors.
	 * 
	 * @see Error
	 */
	...
	/**
	 * Reads the next token and checks that it matches tok. If it does not
	 * match, calls the current error handling routine (see {@link
	 * #set_error_handler(StrTok.Error) set_error_handler()}). If it does match,
	 * just returns.
	 */
