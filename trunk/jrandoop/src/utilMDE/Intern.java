...
public final class Intern {
	...
	/**
	 * Return true if the argument is interned (is canonical among all objects
	 * equal to itself).
	 */
	...
	/**
	 * Hasher object which hashes and compares Integers. This is the obvious
	 * implementation that uses intValue() for the hashCode.
	 * 
	 * @see Hasher
	 */
	...
	/**
	 * Hasher object which hashes and compares Longs. This is the obvious
	 * implementation that uses intValue() for the hashCode.
	 * 
	 * @see Hasher
	 */
	...
	/**
	 * Hasher object which hashes and compares int[] objects according to their
	 * contents.
	 * 
	 * @see Hasher
	...
	/**
	 * Hasher object which hashes and compares long[] objects according to their
	 * contents.
	 * 
	 * @see Hasher
	...
	/**
	 * Hasher object which hashes and compares double[] objects according to
	 * their contents.
	 * 
	 * @see Hasher
	...
	private static final class DoubleArrayHasher implements Hasher {
		public boolean equals(Object a1, Object a2) {
			...
			// Also, it gives inconsistent results (on different
			// JVMs/classpaths?).
			...
			for (int i = 0; i < da1.length; i++) {
				if (!((da1[i] == da2[i]) || (Double.isNaN(da1[i]) && Double
						.isNaN(da2[i])))) {
					return false;
				}
			}
	...
	/**
	 * Hasher object which hashes and compares String[] objects according to
	 * their contents.
	 * 
	...
	/**
	 * Hasher object which hashes and compares Object[] objects according to
	 * their contents.
	 * 
	 * @see Hasher
	 * @see java.util.Arrays#equals(Object[], Object[])
	...
	static {
		internedIntegers = new WeakHasherMap<Integer, WeakReference<Integer>>(
				new IntegerHasher());
		internedLongs = new WeakHasherMap<Long, WeakReference<Long>>(
				new LongHasher());
		internedIntArrays = new WeakHasherMap<int[], WeakReference<int[]>>(
				new IntArrayHasher());
		internedLongArrays = new WeakHasherMap<long[], WeakReference<long[]>>(
				new LongArrayHasher());
		internedDoubles = new WeakHasherMap<Double, WeakReference<Double>>(
				new DoubleHasher());
		internedDoubleNaN = new Double(Double.NaN);
		internedDoubleZero = new Double(0);
		internedDoubleArrays = new WeakHasherMap<double[], WeakReference<double[]>>(
				new DoubleArrayHasher());
		internedStringArrays = new WeakHasherMap<String[], WeakReference<String[]>>(
				new StringArrayHasher());
		internedObjectArrays = new WeakHasherMap<Object[], WeakReference<Object[]>>(
				new ObjectArrayHasher());
		internedIntSequenceAndIndices = new WeakHasherMap<SequenceAndIndices<int[]>, WeakReference<int[]>>(
				new SequenceAndIndicesHasher<int[]>());
		internedLongSequenceAndIndices = new WeakHasherMap<SequenceAndIndices<long[]>, WeakReference<long[]>>(
				new SequenceAndIndicesHasher<long[]>());
		internedDoubleSequenceAndIndices = new WeakHasherMap<SequenceAndIndices<double[]>, WeakReference<double[]>>(
				new SequenceAndIndicesHasher<double[]>());
		internedObjectSequenceAndIndices = new WeakHasherMap<SequenceAndIndices<Object[]>, WeakReference<Object[]>>(
				new SequenceAndIndicesHasher<Object[]>());
		internedStringSequenceAndIndices = new WeakHasherMap<SequenceAndIndices<String[]>, WeakReference<String[]>>(
				new SequenceAndIndicesHasher<String[]>());
	}
	...
	/**
	 * Intern (canonicalize) an Integer. Returns a canonical representation for
	 * the Integer.
	 */
	...
	/**
	 * Intern (canonicalize) a Long. Returns a canonical representation for the
	 * Long.
	 */
	...
	/**
	 * Intern (canonicalize) an int[]. Returns a canonical representation for
	 * the int[] array. Arrays are compared according to their elements.
	 */
	...
	/**
	 * Intern (canonicalize) a long[]. Returns a canonical representation for
	 * the long[] array. Arrays are compared according to their elements.
	 */
	public static long[] intern(long[] a) {
	...
	/**
	 * Intern (canonicalize) a Double. Returns a canonical representation for
	 * the Double.
	 */
	...
	/**
	 * Intern (canonicalize) a double[]. Returns a canonical representation for
	 * the double[] array. Arrays are compared according to their elements.
	 */
	public static double[] intern(double[] a) {
	...
	/**
	 * Intern (canonicalize) an String[]. Returns a canonical representation for
	 * the String[] array. Arrays are compared according to their elements. The
	 * elements should themselves already be interned; they are compared using
	...
	/**
	 * Intern (canonicalize) an Object[]. Returns a canonical representation for
	 * the Object[] array. Arrays are compared according to their elements. The
	 * elements should themselves already be interned; they are compared using
	 * their equals() methods.
	 */
	...
	/**
	 * Convenince method to intern an Object when we don't know its runtime
	 * type. Its runtime type must be one of the types for which we have an
	 * intern() method, else an exception is thrown.
	 */
	public static Object intern(Object a) {
		if (a == null) {
			return null;
			...
			throw new IllegalArgumentException("Arguments of type "
					+ a.getClass() + " cannot be interned");
		}
	}

	/**
	 * Return the subsequence of seq from start (inclusive) to end (exclusive)
	 * that is interned. What's different about this method from manually
	 * findind the subsequence and interning the subsequence is that if the
	 * subsequence is already interned, we can avoid having to compute the
	 * sequence. Since derived variables in Daikon compute the subsequence many
	 * times, this shortcut saves quite a bit of computation. It saves even more
	 * when there may be many derived variables that are non canonical, since
	...
	public static int[] internSubsequence(int[] seq, int start, int end) {
		...
		SequenceAndIndices<int[]> sai = new SequenceAndIndices<int[]>(seq,
				start, end);
		...
		if (lookup != null) {
			...
			internedIntSequenceAndIndices.put(sai, new WeakReference<int[]>(
					subseq));
			return subseq;
		}
	}
	...
	public static long[] internSubsequence(long[] seq, int start, int end) {
		Assert.assertTrue(Intern.isInterned(seq));
		SequenceAndIndices<long[]> sai = new SequenceAndIndices<long[]>(seq,
				start, end);
		...
		if (lookup != null) {
			...
			subseq = Intern.intern(subseq);
			internedLongSequenceAndIndices.put(sai, new WeakReference<long[]>(
					subseq));
			return subseq;
		}
	}
	...
	public static double[] internSubsequence(double[] seq, int start, int end) {
		Assert.assertTrue(Intern.isInterned(seq));
		SequenceAndIndices<double[]> sai = new SequenceAndIndices<double[]>(
				seq, start, end);
		WeakReference<double[]> lookup = internedDoubleSequenceAndIndices
				.get(sai);
		if (lookup != null) {
			return lookup.get();
		} else {
			...
			subseq = Intern.intern(subseq);
			internedDoubleSequenceAndIndices.put(sai,
					new WeakReference<double[]>(subseq));
			return subseq;
		}
	}
	...
	public static Object[] internSubsequence(Object[] seq, int start, int end) {
		Assert.assertTrue(Intern.isInterned(seq));
		SequenceAndIndices<Object[]> sai = new SequenceAndIndices<Object[]>(
				seq, start, end);
		WeakReference<Object[]> lookup = internedObjectSequenceAndIndices
				.get(sai);
		if (lookup != null) {
			return lookup.get();
		} else {
			...
			subseq = Intern.intern(subseq);
			internedObjectSequenceAndIndices.put(sai,
					new WeakReference<Object[]>(subseq));
			return subseq;
		}
	}
	...
	public static String[] internSubsequence(String[] seq, int start, int end) {
		Assert.assertTrue(Intern.isInterned(seq));
		SequenceAndIndices<String[]> sai = new SequenceAndIndices<String[]>(
				seq, start, end);
		WeakReference<String[]> lookup = internedStringSequenceAndIndices
				.get(sai);
		if (lookup != null) {
			return lookup.get();
		} else {
			...
			subseq = Intern.intern(subseq);
			internedStringSequenceAndIndices.put(sai,
					new WeakReference<String[]>(subseq));
			return subseq;
		}
	}

	/**
	 * Data structure for storing triples of a sequence and start and end
	 * indices, to represent a subsequence. Requires that the sequence be
	 * interned. Used for interning the repeated finding of subsequences on the
	 * same sequence.
	 */
	private static final class SequenceAndIndices<T> {
		...
		public boolean equals(SequenceAndIndices<T> other) {
			return (this.seq == other.seq) && this.start == other.start
					&& this.end == other.end;
		}
	...
	private static final class SequenceAndIndicesHasher<T> implements Hasher {
		public boolean equals(Object a1, Object a2) {
			return ((SequenceAndIndices<T>) a1)
					.equals((SequenceAndIndices<T>) a2); // unchecked cast
		}
