...
/**
 * 
 * <b>RandomSelector</b> selects k elements uniformly at random from an
 * iteration over n elements using only O(k) space instead of O(n) space. For
 * example, selecting 1 element from a FileStream containing 1000 elements will
 * take O(1) space. The class takes as input the number k during initialization
 * and then can accept() any number of Objects in the future. At any point in
 * time, getVariables() will either return k randomly selected elements from the
 * elements previous accepted or if accept() was called fewer than k times, will
 * return all elements previously accepted.
 * 
 * <p>
 * The random selection is independent between every constructed instance of
 * RandomSelector objects, but for the same instance, multiple calls to
 * getVariables() are not independent. Making two calls to consecutive
 * getVariables() without an accept() in between will return two new Lists
 * containing the same elements.
 * 
 * <p>
 * A second mode allows for a fixed probability of randomly keeping each item as
 * opposed to a fixed number of samples.
 * 
 * <P>
 * SPECFIELDS: <BR>
 ...
 * selector_mode : {FIXED,PERCENT} : either fixed amount of samples or fixed
 * percent.
 * 
 * <P>
 * Example use: <BR> // randomly selects 100 lines of text from a file
 * 
 * <pre>
...
public class RandomSelector<T> {
	...
	/**
	 * @param num_elts
	 *            The number of elements intended to be selected from the input
	 *            elements
	 * 
	 ...
	 *            The probability that each element is selected from the
	 *            oncoming Iteration.
	 * @param r
	 *            The seed to give for random number generation.
	 */
	...
	/**
	 * <P>
	 * When in fixed sample mode, increments the number of observed elements i
	 * by 1, then with probability k / i, the Object 'next' will be added to the
	 * currently selected values 'current_values' where k is equal to
	 * 'number_to_take'. If the size of current_values exceeds number_to_take,
	 * then one of the existing elements in current_values will be removed at
	 * random.
	 * 
	 * 
	 * <P>
	 * When in probability mode, adds next to 'current_values' with probability
	 * equal to 'keep_probability'.
	 * 
	 */
