...
public final class MathMDE {
	...
	public static long pow_fast(long base, long expt)
	...
	/**
	 * Return the gcd (greatest common divisor) of the differences between the
	 * elements of int array a.
	 */
	...
	/**
	 * Return an array of two integers (r,m) such that each number in NUMS is
	 * equal to r (mod m). The largest possible modulus is used, and the trivial
	 * constraint that all integers are equal to 0 mod 1 is not returned (null
	 * is returned instead).
	 */
	...
	/**
	 * The iterator produces Integer values. This can be more efficient than
	 * modulus(int[]) if the int[] doesn't already exist, because this does not
	 * necessarily examine every value produced by its iterator.
	 */
	 ...
	 * <p>
	 * 
	 * This "_strict" version differs from the regular modulus by requiring that
	 * the argument be dense: that is, every pair of numbers in the argument
	 * array is separated by exactly the modulus.
	 * <p>
	 * 
	 * The endpoints can be treated in two different ways: Either exactly like
	 * other numbers in the input, or they can merely be checked for the
	 ...
	 */
	public static int[] modulus_strict_int(Iterator<Integer> itor,
	...
	/**
	 * The iterator produces Long values. This can be more efficient than
	 * modulus(long[]) if the long[] doesn't already exist, because this does
	 * not necessarily examine every value produced by its iterator.
	 */
	 ...
	 * @see #modulus_strict(int[], boolean)
	 */
	public static long[] modulus_strict_long(Iterator<Long> itor,
			boolean nonstrict_ends) {
		if (!itor.hasNext())
			return null;
	 ...
	 * but in the argument's range; that is, bigger than its argument's minimum
	 * value and smaller than its argument's maximum value. The result contains
	 * no duplicates and is in order.
	 */
	...
	/**
	 * This iterator returns all the numbers *not* in its argument array but in
	 * the argument's range; that is, bigger than its argument's minimum value
	 * and smaller than its argument's maximum value. The result contains no
	 * duplicates and is in order. If boolean add_ends is set, then the
	 * bracketing endpoints are also returned; otherwise, all returned values
	 * are between the minimum and maximum of the original values.
	 */
	static final class MissingNumbersIteratorInt implements Iterator<Integer> {
		...
		public boolean hasNext() {
			...
			if (add_ends) {
				...
				throw new Error("Can't happen: " + current_missing + " "
						+ current_nonmissing);
			}
		}
	...
	/**
	 * Return a tuple of (r,m) where no number in NUMS is equal to r (mod m) but
	 * all missing numbers in their range are.
	 */
	public static int[] nonmodulus_strict(int[] nums) {
		...
		if (range > 65536)
			return null;
		return nonmodulus_strict_int_internal(new MissingNumbersIteratorInt(
				nums, true));
	}
	...
	private static int[] nonmodulus_strict_int_internal(
		...
		// modulus_strict.
		UtilMDE.RemoveFirstAndLastIterator<Integer> missing_nums = new UtilMDE.RemoveFirstAndLastIterator<Integer>(
				missing);
	...
	private static boolean check_first_and_last_nonmodulus(int[] rm,
	...
	/**
	 * Return a tuple of (r,m) where no number in NUMS is equal to r (mod m) but
	 * for every number in NUMS, at least one is equal to every non-r remainder.
	...
	public static int[] nonmodulus_nonstrict(int[] nums) {
		if (nums.length < 4)
			return null;
		int max_modulus = Math.min(nums.length / 2, ArraysMDE
				.element_range(nums) / 2);

		// System.out.println("nums.length=" + nums.length + ", range=" +
		// ArraysMDE.element_range(nums) + ", max_modulus=" + max_modulus);
		...
		for (int m = 2; m <= max_modulus; m++) {
			...
			for (int i = 0; i < nums.length; i++) {
				...
				if (!has_modulus[rem]) {
					...
					num_nonmodulus--;
					// System.out.println("rem=" + rem + " for " + nums[i] + ";
					// num_nonmodulus=" + num_nonmodulus);
					if (num_nonmodulus == 0)
						// Quit as soon as we see every remainder instead of
						// processing
						// each element of the input list.
						break;
				}
			}
			// System.out.println("For m=" + m + ", num_nonmodulus=" +
			// num_nonmodulus);
			if (num_nonmodulus == 1) {
	...
	public static long[] missing_numbers(long[] nums) {
		...
		long[] result = new long[new Long(max - min + 1 - nums.length)
				.intValue()];
		int result_index = 0;
		long val = min;
		for (int i = 0; i < nums.length; i++) {
	...
	public static long[] nonmodulus_strict(long[] nums) {
		...
		if (range > 65536)
			return null;
		return nonmodulus_strict_long_internal(new MissingNumbersIteratorLong(
				nums, true));
	}
	...
	private static long[] nonmodulus_strict_long_internal(Iterator<Long> missing) {
		...
		// nonmodulus_strict({1,2,3,5,6,7,9,11}) => {0,2}. Thus, use
		// modulus_strict.
		UtilMDE.RemoveFirstAndLastIterator<Long> missing_nums = new UtilMDE.RemoveFirstAndLastIterator<Long>(
				missing);
	...
	private static boolean check_first_and_last_nonmodulus(long[] rm,
			UtilMDE.RemoveFirstAndLastIterator<Long> rfali) {
		long r = rm[0];
		long m = rm[1];
	...
	public static long[] nonmodulus_nonstrict(long[] nums) {
		if (nums.length < 4)
			return null;
		int max_modulus = new Long(Math.min(nums.length / 2, ArraysMDE
				.element_range(nums) / 2)).intValue();
