...
/**
 * LimitedSizeIntSet stores up to some maximum number of unique non-zero integer
...
public class LimitedSizeIntSet implements Serializable, Cloneable {
	...
	public void addAll(LimitedSizeIntSet s) {
		...
		if (s.repNulled()) {
			if (size() + s.size() >= values.length) {
				...
				throw new Error(
						"Arg is rep-nulled, but isn't enough to rep-null this.");
			}
		}
	...
	/**
	 * A lower bound on the number of elements in the set. Returns either the
	 * number of elements that have been inserted in the set, or max_size(),
	 * whichever is less.
	 */
	public int size() {
		return num_values;
	...
	/**
	 * An upper bound how many distinct elements can be individually represented
	 * in the set. Returns max_values+1 (where max_values is the argument to the
	 * constructor).
	 */
	public int max_size() {
		if (values == null) {
	 ...
	 * represents the values seen by the entire list. Returns the new object,
	 * whose max_values is the given integer.
	 */
