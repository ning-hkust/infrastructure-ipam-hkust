 ...
 * sequence/iterator/whatever which pairs the matching elements of the inputs.
 * 
 * Requires that the elements in the input iterators be sorted in the same
 * order.
 * 
 * It's not clear that this is always the right thing to do; you might find it
 * more convenient to use use set intersection/difference.
 */
	...
	public Pair<T, T> next() {
		if (next1 == null) {
			...
			if (next2 == null) {
				...
				int comparison;
				// Either T extends Comparable<T>, or else a comparator was
				// passed in.
				if (comparator == null) {
					comparison = ((Comparable<T>) next1).compareTo(next2); // unchecked
																			// cast
				} else {
