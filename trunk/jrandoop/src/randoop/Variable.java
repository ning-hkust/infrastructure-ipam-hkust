...
public class Variable implements Comparable<Variable>, Serializable {
	...
	public Object getValue() {
		Object obj = null;
		StatementKind sk = sequence.getStatementKind(index);
		ExecutionOutcome exeOut = sequence.getResult(index);
		
		if (exeOut == null) {
			if (sk instanceof PrimitiveOrStringOrNullDecl) {
				obj = ((randoop.PrimitiveOrStringOrNullDecl) sk).getValue();

			} else if (sk instanceof ArrayDeclaration) {
				//((randoop.ArrayDeclaration) sk).
				obj = null;
			} else if (sk instanceof RConstructor) {
				obj = null;
			} else if (sk instanceof RMethod) {
				//((randoop.RMethod) sk).getOutputType()
				obj = null;
			}
		}
		else
			obj = ((NormalExecution) exeOut).getRuntimeValue();
