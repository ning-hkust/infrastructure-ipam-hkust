...
public class BranchDirMutator {
		...
		for (int i = 0; i < s.size(); i++) {
			...
			for (int j = 0; j < st.inputs.size(); j++) {
				if (st.inputs.get(j).equals(oldv)) {
					assert Reflection.canBeUsedAs(newv.getType(),
							st.statementKind.getInputTypes().get(j));
				}
			}
		}
	}
	...
	private static String complexReplace(MSequence s, MVariable oldv,
			MVariable newv) {
	...
	private static String simpleReplace(MSequence s, MVariable oldv,
			MVariable newv) {
		assert oldv.getDeclIndex() > newv.getDeclIndex();
		for (int i = 0; i < s.size(); i++) {
			MStatement st = s.statements.get(i);
