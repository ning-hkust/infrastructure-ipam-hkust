...
public class HelperSequenceCreator {
	 ...
	 * an array.
	 */
	public static SimpleList<Sequence> createSequence(Class<?> cls,
			SequenceCollection components) {

		assert !components.hasSequences(cls, Match.COMPATIBLE_TYPE);
		...
		if (cls.getComponentType().isPrimitive()) {
			...
			if (candidates.isEmpty()) {
				...
				// implementation.
				ins.add(s.randomVariableForTypeLastStatement(cls
						.getComponentType(), Match.COMPATIBLE_TYPE));
				s = s.extend(decl, ins);
			}
		}
		assert s != null;
