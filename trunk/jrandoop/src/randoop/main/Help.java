...
public class Help extends CommandHandler {
	...
	public boolean handle(String[] args) throws RandoopTextuiException {
		...
		if (args.length == 0) {
			...
			try {
				...
				System.out
						.println("Error while reading file containing Randoop's version number.");
				System.out.println(e.getMessage());
				System.exit(1);
			}
			out.println();
			out
					.println("Type `help' followed by a command name to see documentation.");
			out.println();
			out.println("Commands:");
			out.println();
			for (CommandHandler h : Main.handlers) {
				out.println(Util.hangingParagraph(h.fcommand + " -- "
						+ h.fpitch, Globals.COLWIDTH, Globals.INDENTWIDTH));
			}
			out.println();
	...
	protected static void introMessage(PrintStream out) {
		...
		out.println();
		out
				.println("Randoop  is a command-line tool that creates unit tests for Java.");
		out
				.println("It accepts one of the commands listed below. For the user manual,");
		out
				.println("please visit http://people.csail.mit.edu/cpacheco/randoop/");
	}
}
