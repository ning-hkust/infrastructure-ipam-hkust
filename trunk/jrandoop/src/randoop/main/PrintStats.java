...
public class PrintStats {
	...
	@Option("If given, prints all the branches to the giben file.")
	...
	public static void main(String[] args) throws IOException {
		...
		if (cov_classes == null) {
			System.out
					.println("Missing required option --cov-classes=<filename>");
			System.exit(1);
		}
		if (stats_file.isEmpty()) {
			System.out
					.println("Missing required option --stats-file=<filename>");
			System.exit(1);
		}
		...
		for (String stat : stats_file) {
			...
			if (!resultsFile.exists()) {
				System.out.println("DID NOT FIND FILE (WILL SKIP): "
						+ resultsFile);
				continue;
			}
			SequenceGeneratorStats run = (SequenceGeneratorStats) SerializationHelper
					.readSerialized(resultsFile);
			runs.add(run);
		}
