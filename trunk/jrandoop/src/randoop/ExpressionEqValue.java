...
/**
 * An observation recording the value of an expression evaluated to during
 * execution. In particular, this observation represents
 * 
...
public class ExpressionEqValue implements Observation, Serializable {
	...
	/**
	 * @param expression
	 *            The expression. Cannot be not null.
	 * @param vars
	 *            The variables for the expression. Must all belong to the same
	 *            sequence.
	 * @param value
	...
	public ExpressionEqValue(Class<? extends Expression> expression,
		...
		if (value != null
				&& !PrimitiveTypes.isBoxedPrimitiveTypeOrString(value
						.getClass()))
	...
	public String toCodeStringPostStatement() {
		...
		b
				.append("// Regression assertion (captures the current behavior of the code)"
						+ Globals.lineSep);
		...
		if (expression.equals(ValueExpression.class)) {
			...
			if (value == null) {
				...
				String arraystr = PrimitiveTypes.toArrayCodeString(value);
				if (arraystr != null)
				{
					b.append(var);
					b.append(".equals(new Object[]" );
					b.append(arraystr);
					b.append(")");
				} else
				{
