...
public final class ClassComplexityCalculator {
	...
	public ClassComplexityCalculator(Collection<Class<?>> classes) {
		...
		h = new ClassHierarchy(ClassHierarchy
				.superClassClosure(new LinkedHashSet<Class<?>>(Reflection
						.relatedClasses(classes, 1))));
		computeForAll();
	}
	...
	public static void main(String[] args) throws ClassNotFoundException {
		...
		for (String className : args) {
			String classNa = className.substring(0, className.length()
					- ".class".length());
			System.out.println("loading " + classNa + " " + (classCount++)
					+ " of " + args.length);
			try {
