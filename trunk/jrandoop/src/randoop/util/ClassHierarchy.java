...
public final class ClassHierarchy {
	...
	public ClassHierarchy(Collection<Class<?>> classes) {
		...
		this.classes = Collections.unmodifiableSet(new LinkedHashSet<Class<?>>(
				classes));// copy
	...
	private Collection<Class<?>> internalSuperClasses(Class<?> clazz) {
		if (classToSuperclasses.contains(clazz)) // already computed
			return classToSuperclasses.getVariables(clazz);
	...
	public Set<Class<?>> superClasses(Class<?> c) {
		return Collections.unmodifiableSet(classToSuperclasses.getVariables(c));
	}

	public Set<Class<?>> subClasses(Class<?> c) {
		if (classToSubclasses.contains(c)) {
			// System.out.println("subClasses:" + c.getName() + " precomputed");
			return Collections.unmodifiableSet(classToSubclasses
					.getVariables(c));
		}
		Set<Class<?>> result = new LinkedHashSet<Class<?>>();
	...
	/**
	 * Returns the set of all direct and indirect superclasses and
	 * superinterfaces of the given class. The returned set includes also the
	 * class itself.
	 */
