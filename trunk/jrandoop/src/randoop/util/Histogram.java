...
public final class Histogram<T> {
	private final Map<T, Integer> fMap = new LinkedHashMap<T, Integer>();
	...
	private String entriesToString(boolean append_percent,
			Comparator<Map.Entry<T, Integer>> c) {
		List<Map.Entry<T, Integer>> entries = new ArrayList<Map.Entry<T, Integer>>(
				fMap.entrySet());
		...
		for (Iterator<Map.Entry<T, Integer>> iter = entries.iterator(); iter
				.hasNext();) {
